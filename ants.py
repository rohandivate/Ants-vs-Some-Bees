...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                if insect.can_contain(self.ant) and insect.container == True:
                    insect.contain_ant(self.ant)
                    self.ant, insect.place = insect, self
                    return None
                elif self.ant.can_contain(insect) and self.ant.container == True:
    ...
    def remove_insect(self, insect):
        ...
        """
        if isinstance(insect, QueenAnt) and insect.exists:
            return None
        if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while(curr_pos):
            if len(curr_pos.bees) != 0 and curr_pos.name != 'Hive' and (self.min_range <= range_index) and (range_index <= self.max_range):
                return random_or_none(curr_pos.bees)
            else:
                range_index += 1
                curr_pos = curr_pos.entrance
        # END Problem 3B
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 9
        Ant.__init__(self)
        if QueenAnt.not_impostor:
            QueenAnt.not_impostor = False
            self.exists = True
            self.ants_list = []
        else: 
            self.exists = False
    ...
    def action(self, colony):
        ...
        def double_damage(ant):
            if not ant:
                return None
            elif not ant in self.ants_list:
                ant.damage *= 2
                self.ants_list.append(ant)

            if ant.container:
                contained_ant = ant.ant
                if (contained_ant) and (not contained_ant in self.ants_list):
                    contained_ant.damage *= 2
                    self.ants_list.append(contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    curr_action = effect(temp_action)
